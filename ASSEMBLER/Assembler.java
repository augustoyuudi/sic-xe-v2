...
public class Assembler {
	...
	private Code c = new Code(); //Class to contain the inputs
	private Integer LOCCTR;  
	private Symtab SYMTAB = new Symtab(); //Symbol Table
	...
	//Main method for the whole assemble process to send to loader 
	public ArrayList<String> handleInputFiles(File[] selectedFiles) {
		String strCode;
		MacroProcessor mc = new MacroProcessor();
		
		for (File file : selectedFiles) {
			strCode = c.readInputFile(file.getAbsolutePath());
			code = mc.processCode(strCode);
			objectPrograms.add(assemble());
		}

		Linker linker = new Linker(objectPrograms);
		ArrayList<String> a = linker.linkCodes();
		System.out.print(a);
		return a;
		//return linker.linkCodes();
	}
	
	//Returns the result of the two passes assembler
	public ArrayList<String> assemble() {
		passOne(); passTwo();
		return objectProgram;
	}

	// pass one method of assembler
	...
	// Handler method for format2 instructions
	...
	// Handler method for format3 instructions
